...
// Drop at: frontend/src/features/sales/components/StageStepper.tsx
// ============================================================
...
interface StageStepperProps {
  ...
  visitLabel?: string;          // e.g. "Apr 19 2pm"
}
      ...
      <div className="flex items-start">
        {STAGES.map((s, i) => {
          const state: StepState =
            i < currentIdx ? 'done'
            : i === currentIdx
              ? (s.key === 'pending_approval' ? 'waiting' : 'active')
              : 'future';
...
function Step({
  ...
  const dotClass = {
    ...
    waiting: 'border-2 border-dashed border-amber-400 text-amber-600 bg-amber-50 animate-pulse',
...
function Connector({ done }: { done: boolean }) {
  return (
    <div
      className={`h-[2px] flex-1 mt-[13px] ${done ? 'bg-emerald-500' : 'bg-slate-200'}`}
    />
  );
}
