/**
 * Pure presentational estimate form — line-item builder + template picker.
 *
 * Extracted from ``EstimateCreator`` so the same form can be mounted by
 * the appointment-modal wrapper (creates an Estimate via the appointment
 * endpoint) and by the Sales Pipeline wrapper (creates + sends via the
 * pipeline orchestrator). Hooks can't be called conditionally, so each
 * wrapper injects its own mutation through ``onSubmit``.
 */
...
const emptyLineItem: EstimateFormLineItem = {
  item: '',
  description: '',
  unit_price: 0,
  quantity: 1,
...
export function EstimateForm({
  ...
  const [lineItems, setLineItems] = useState<EstimateFormLineItem[]>([
    { ...emptyLineItem },
  ]);
  ...
  const handlePickerAdd = (draft: PickerLineItemDraft) => {
    const next: EstimateFormLineItem = {
      item: draft.item,
  ...
  const updateLineItem = (
    index: number,
    field: keyof EstimateFormLineItem,
    ...
    <div data-testid="estimate-creator" className="space-y-3 p-3 bg-slate-50 rounded-xl">
      ...
      <div className="flex justify-end">
        <p
          ...
          data-testid="appt-estimate-total"
        >
