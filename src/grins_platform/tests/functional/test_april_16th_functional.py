...
class TestNotesTimeline:
    """Functional tests for notes timeline operations."""

    def test_note_create_schema_validation(self) -> None:
        """NoteCreate validates body is non-empty and within limits."""
        from grins_platform.schemas.note import NoteCreate

        # Valid
        note = NoteCreate(body="This is a test note")
        ...
        # Too long body rejected
        with pytest.raises(ValidationError):
            NoteCreate(body="x" * 10001)

    def test_note_response_stage_tags(self) -> None:
        """NoteResponse correctly maps subject_type to stage_tag."""
        ...
        for subject_type, expected_tag in stage_map.items():
            response = NoteResponse(
                ...
                created_at=now,
                updated_at=now,
                stage_tag=expected_tag,
            )
            assert response.stage_tag == expected_tag

    @pytest.mark.asyncio
    async def test_note_service_create_sets_origin_lead_id_for_leads(
        self,
    ) -> None:
        """Creating a note on a lead auto-sets origin_lead_id to the lead ID."""
        ...
        # Mock the Note model creation
        with patch("grins_platform.services.note_service.Note") as MockNote:
            mock_note = MagicMock()
            mock_note.id = uuid4()
            mock_note.subject_type = "lead"
            mock_note.subject_id = lead_id
            mock_note.author_id = author_id
            mock_note.body = "Test note"
            mock_note.origin_lead_id = lead_id
            mock_note.is_system = False
            mock_note.is_deleted = False
            mock_note.created_at = datetime.now(tz=timezone.utc)
            mock_note.updated_at = datetime.now(tz=timezone.utc)
            mock_note.author = MagicMock()
            mock_note.author.name = "Test Author"
            MockNote.return_value = mock_note
            ...
            # Verify origin_lead_id was set to lead_id
            assert MockNote.call_args.kwargs.get("origin_lead_id") == lead_id or \
                   MockNote.call_args[1].get("origin_lead_id") == lead_id
