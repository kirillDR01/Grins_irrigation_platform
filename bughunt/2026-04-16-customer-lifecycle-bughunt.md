...
|---|---|---|---|---|
| CR-1 — `apply_schedule` creates SCHEDULED not DRAFT | 2026-04-16 | `2f7caaf` | `fix/cr-1-apply-schedule-draft` | Default to `AppointmentStatus.DRAFT.value`; `Job.status` stays `to_be_scheduled`. |
| CR-2 — `job_started` doesn't promote SCHEDULED → IN_PROGRESS | 2026-04-16 | `c65283e` | `fix/cr-2-job-started-scheduled` | Added `SCHEDULED` to pre-state set; transitions table now allows `SCHEDULED → IN_PROGRESS`. |
| CR-3 — Repeat `C` sends duplicate cancellation SMS | 2026-04-16 | `9a30fac` | `fix/cr-3-repeat-cancel-sms` | `_handle_cancel` short-circuits when already CANCELLED; empty `auto_reply` suppresses SMS. |
| CR-4 — `invoice.paid` misclassifies non-first invoices as renewals | 2026-04-16 | `6d99587` | `fix/cr-4-invoice-billing-reason` | Renewal branch now gated on Stripe `billing_reason == "subscription_cycle"`. |
| CR-5 — Lien SMS sent immediately instead of queued | 2026-04-16 | `d679c25` | `fix/cr-5-lien-review-queue` | Split into `compute_lien_candidates` + `send_lien_notice` with SMS-consent pre-filter and audit log; new `/invoices?tab=lien-review` queue UI. `mass_notify("lien_eligible")` now returns 400. |
| CR-6 — `convert_lead` skips Tier-1 dedup | 2026-04-16 | `7007002` | `fix/cr-6-convert-lead-dedup` | Added `force` flag; raises `LeadDuplicateFoundError` → 409 with structured detail; FE surfaces conflict modal with "Use existing" + "Convert anyway". |
