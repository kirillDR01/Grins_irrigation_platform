...
| M-1 — `AppointmentDetail` missing draft Send button | 2026-04-16 | `d1fb2dc` | `fix/medium-bughunt-batch` | Inline draft button replaced with canonical `SendConfirmationButton` so the modal shares the calendar card's UX (toast + isPending styling). |
| M-2 — Cancel dialog collapses to one button on DRAFT | 2026-04-16 | `213f667` | `fix/medium-bughunt-batch` | Both buttons stay visible; native `title` tooltip ("Draft was never sent, no text needed.") explains the disabled "Cancel & text" variant. |
| M-3 — Y/R/C keyword map missing common synonyms | 2026-04-16 | `1cf1eed` | `fix/medium-bughunt-batch` | Added `ok/okay/yup/yeah/1` (CONFIRM) and `different time/change time/2` (RESCHEDULE). `stop` and `3` intentionally excluded. |
| M-4 — Confirm auto-reply lacks date/time | 2026-04-16 | `1cf1eed` | `fix/medium-bughunt-batch` | New `_build_confirm_message` helper renders `"Your appointment has been confirmed. See you on {date} at {time}!"` with graceful fallback. |
| M-5 — Reschedule ack diverges from spec | 2026-04-16 | `1cf1eed` | `fix/medium-bughunt-batch` | Spec-exact wording: `"We've received your reschedule request. We'll be in touch with a new time."`. |
| M-6 — Google Review SMS uses wrong wording | 2026-04-16 | `083df11` | `fix/medium-bughunt-batch` | Spec-exact text; dropped first-name prefix and `Grin's Irrigations` typo. |
| M-7 — Admin appointment edits don't audit | 2026-04-16 | `6b4403b` | `fix/medium-bughunt-batch` | Two new helpers (`_record_update_audit`, `_record_reactivate_audit`) capture pre/post snapshots, `notify_customer`, and `actor_id`. Wired through `update_appointment` API. |
| M-8 — Customer-SMS cancel doesn't audit | 2026-04-16 | `c89f78d` | `fix/medium-bughunt-batch` | New `_record_customer_sms_cancel_audit` writes `appointment.cancel` row with `source="customer_sms"` whenever the C reply actually transitioned (skipped on CR-3 short-circuit). |
| M-9 — Auto-reply + follow-up bypass SentMessage | 2026-04-16 | `9ba1d6d` | `fix/medium-bughunt-batch` | Both routes now go through `SMSService.send_message` with new `APPOINTMENT_CONFIRMATION_REPLY` + `RESCHEDULE_FOLLOWUP` MessageType values. Migration `20260416_100400` (renumbered post-merge from `20260416_100000` to avoid the H-4 clash) widens the CHECK constraint. |
| M-10 — Convert button takes two round-trips on SEND_CONTRACT | 2026-04-16 | `86b0c91` | `fix/medium-bughunt-batch` | Behavior already correct in dev; new regression test locks convert-first ordering. |
| M-11 — `_get_signing_document` legacy fallback unsafe | 2026-04-16 | `c0a7026` | `fix/medium-bughunt-batch` | Strict entry-scope by default; legacy `sales_entry_id IS NULL` fallback gated behind `include_legacy=True` (reporting reads only). |
| M-12 — `days_until_due` / `days_past_due` computed client-side | 2026-04-16 | `fade767` | `fix/medium-bughunt-batch` | Pydantic `computed_field`s on `InvoiceResponse` (UTC, mutually exclusive); FE drops its `daysDiff` branch. |
| M-13 — `PARTIALLY_APPROVED` only fires when no PENDING | 2026-04-16 | `c6bed1c` | `fix/medium-bughunt-batch` | Now fires whenever APPROVED + (REJECTED or PENDING) coexist; `REJECTED + PENDING` (no approvals) keeps PENDING. |
| M-14 — Mass-notify template field names off-spec | 2026-04-16 | `7730738` | `fix/medium-bughunt-batch` | New `render_invoice_template()` + `validate_invoice_template()`; canonical `{customer_name}/{invoice_number}/{amount}/{due_date}` plus spec-bracket forms. Invalid admin template → 400 with named missing keys. |
| M-15 — Stripe webhook 400 on missing secret | 2026-04-16 | `1fbc1bc` | `fix/medium-bughunt-batch` | 503 instead of 400 so Stripe retries with backoff and the ERROR-level log pages on-call. |
| M-16 — Upload size/MIME validation frontend-only | 2026-04-16 | `0db359f` | `fix/medium-bughunt-batch` | `validate_file` raises `TypeError` for MIME (→ 415) vs. `ValueError` for size (→ 413); 4 upload endpoints map both correctly. |
| M-17 — `DocumentsSection` doesn't validate presign | 2026-04-16 | `fdb1f7a` | `fix/medium-bughunt-batch` | New `useDocumentPresign` resolves URL at render; signing buttons disable with explanatory tooltip when file_key is missing/expired. |
...
| M-9 silence pyright unused-result on audit/SMS sends | `9381543` | `_ = await ...` pattern on the new audit + send_message calls so pyright warning count stays at the dev baseline. |
