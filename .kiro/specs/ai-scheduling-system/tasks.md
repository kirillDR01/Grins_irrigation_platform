...
> **Spec drift updates (2026-04-28):** This spec was originally authored 2026-04-14. A drift audit on 2026-04-28 surfaced three items below that are now reflected throughout the spec:
> 1. A **generic alerts infrastructure** (model, table, router, service) was added 2026-04-16 for SMS-cancellation/H-5 bughunt work. The scheduling-specific work uses **`scheduling_alerts.py` / `/api/v1/scheduling-alerts/`** to avoid colliding with the existing `alerts.py` / `/api/v1/alerts/`.
> 2. **`GET /api/v1/schedule/capacity` already exists** (basic daily capacity). Task 7.3 *extends* it with 30-criteria fields rather than creating a new endpoint.
> 3. The **correctness-property count is 27, not 22** (Properties 1–22 backend Hypothesis, Properties 23–27 frontend fast-check). All references in design.md and tasks.md are now consistent at 27.
>
> No commits between 2026-04-14 and 2026-04-28 implemented any AI-scheduling code, so all backend foundations and "missing" frontend directories called out below are still accurate.
...
- **Generic Alert infrastructure already exists** (added 2026-04-16): `models/alert.py` (generic Alert model for SMS-cancellation/H-5 alerts), `migrations/versions/20260416_100100_create_alerts_table.py`, `api/v1/alerts.py` (router at `/api/v1/alerts/`), `services/alert_service.py`. The new scheduling-specific work in this spec uses **`models/scheduling_alert.py` / `scheduling_alerts` table / `api/v1/scheduling_alerts.py` / `/api/v1/scheduling-alerts/`** to avoid name collision. Do not modify the generic alerts files.
- **`GET /api/v1/schedule/capacity` already exists** in `api/v1/schedule.py` as the `get_capacity` handler. Task 7.3 extends this endpoint additively with 30-criteria analysis fields; it does NOT create a new `/capacity` endpoint and must not break the existing response shape.
...
- [x] 7. API routes — AI scheduling, alerts, and schedule extensions
  ...
  - [x] 7.2 Create scheduling-alerts API router
    - Create `src/grins_platform/api/v1/scheduling_alerts.py` — note the deliberate filename: a generic `alerts.py` already exists (added 2026-04-16 for SMS-cancellation/H-5 alerts) and must not be modified by this work.
    - Implement `router = APIRouter(prefix="/scheduling-alerts", tags=["scheduling-alerts"])` (NOT `prefix="/alerts"` — that prefix is already taken by the generic alerts router at `api/v1/alerts.py`)
  ...
  - [x] 7.3 Extend existing schedule API router
    ...
    - Add `GET /utilization` — resource utilization report, returns `UtilizationReport` (new endpoint)
  ...
  - [x] 7.4 Register new routers in main API router
    ...
    - Import and include `scheduling_alerts_router` (from `api.v1.scheduling_alerts`) with `prefix="/scheduling-alerts"`, `tags=["scheduling-alerts"]`. The pre-existing generic `alerts_router` (from `api.v1.alerts`) at `prefix="/alerts"` remains untouched.
...
- [x] 10. Frontend — Alerts/Suggestions Panel
  ...
  - [x] 10.1 Create AlertsPanel component
    ...
    - Fetch alerts via `GET /api/v1/scheduling-alerts/` with polling (configurable interval, default 30s)
  ...
  - [x] 10.2 Create AlertCard component
    ...
    - Resolution actions call `POST /api/v1/scheduling-alerts/{id}/resolve`
  ...
  - [x] 10.3 Create SuggestionCard component
    ...
    - Accept calls `POST /api/v1/scheduling-alerts/{id}/resolve`, dismiss calls `POST /api/v1/scheduling-alerts/{id}/dismiss`
  ...
  - [x] 10.5 Create ChangeRequestCard component
    ...
    - Approve calls `POST /api/v1/scheduling-alerts/change-requests/{id}/approve`
    - Deny calls `POST /api/v1/scheduling-alerts/change-requests/{id}/deny` with reason input
  ...
  - [x] 10.6 Create TanStack Query hooks for alerts
    ...
    - `useAlerts(params)` — query `GET /api/v1/scheduling-alerts/` with polling refetchInterval
    - `useResolveAlert()` — mutation `POST /api/v1/scheduling-alerts/{id}/resolve`
    - `useDismissAlert()` — mutation `POST /api/v1/scheduling-alerts/{id}/dismiss`
    - `useChangeRequests()` — query `GET /api/v1/scheduling-alerts/change-requests`
    - `useApproveChangeRequest()` — mutation `POST /api/v1/scheduling-alerts/change-requests/{id}/approve`
    - `useDenyChangeRequest()` — mutation `POST /api/v1/scheduling-alerts/change-requests/{id}/deny`
...
- [x] 11. Frontend — AI Chat extensions
  ...
  - [x] 11.5 Create root barrel export for AI feature
...
- [x] 13A. Page composition and routing — Compose components into page views and register routes
  ...
  - [x] 13A.2 Export `AIScheduleView` from `frontend/src/features/schedule/index.ts`
    - _Requirements: 40.1_

  - [x] 13A.3 Update `frontend/src/pages/ScheduleGenerate.tsx` to render `AIScheduleView`
  ...
  - [x] 13A.4 Create ResourceMobileView composed page component
  ...
  - [x] 13A.5 Export `ResourceMobileView` from `frontend/src/features/resource-mobile/index.ts`
    - _Requirements: 40.4_

  - [x] 13A.6 Create `frontend/src/pages/ScheduleMobile.tsx` page wrapper
    - Follow the thin page-wrapper pattern: import `ResourceMobileView` from `@/features/resource-mobile` and render it
    - _Requirements: 39.2_

  - [x] 13A.7 Add `/schedule/mobile` route to `frontend/src/core/router/index.tsx`
  ...
  - [x] 13A.8 Verify page composition compiles
    - Run TypeScript type checking (`npx tsc --noEmit`) and ESLint on all new/modified files

  - [x] 13A.9 Write unit tests for composed pages
  ...
  - [x] 13A.10 Write property-based tests (fast-check) for Properties 23–27
...
- [x] 23. Code quality, linting, type safety, and simulation testing
  - [x] 23.1 Run and fix Ruff linting on all scheduling code
    - Run `uv run ruff check --fix src/grins_platform/services/ai/scheduling/ src/grins_platform/api/v1/ai_scheduling.py src/grins_platform/api/v1/scheduling_alerts.py src/grins_platform/models/scheduling_*.py src/grins_platform/models/change_request.py src/grins_platform/models/service_zone.py src/grins_platform/models/resource_truck_inventory.py src/grins_platform/schemas/ai_scheduling.py`
  ...
  - [x] 23.2 Run and fix MyPy type checking on all scheduling code
    - Run `uv run mypy src/grins_platform/services/ai/scheduling/ src/grins_platform/api/v1/ai_scheduling.py src/grins_platform/api/v1/scheduling_alerts.py src/grins_platform/models/ src/grins_platform/schemas/ai_scheduling.py`
  ...
  - [x] 23.3 Run and fix Pyright type checking on all scheduling code
    - Run `uv run pyright src/grins_platform/services/ai/scheduling/ src/grins_platform/api/v1/ai_scheduling.py src/grins_platform/api/v1/scheduling_alerts.py`
    - Zero errors required
    - _Requirements: 30.4_
